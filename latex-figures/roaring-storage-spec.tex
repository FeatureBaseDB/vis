...
\begin{tikzpicture}
  ...
  \block[] {
    ...
    \block[minimum width=16em]{
      ...
      \hskip 2em 4 \block[minimum width=12em]{Cookie}\\
      % "\," is a space
      \hskip 2em 4 \block[minimum width=12em]{$\textcolor{pilosa-red}{N_c}$\textcolor{pilosa-blue}{\, = Container count}}\\
      \hskip 0.3em \textcolor{pilosa-blue}{$\frac{\textcolor{pilosa-red}{N_c}+7}{8}$} \block[minimum width=12em]{runFlagBitset}
    }\\
    \block[minimum width=16em]{
      Descriptive header\\
      \block[minimum width=15em]{
        2 \block[minimum width=13em]{Key 0}\\
        2 \block[minimum width=13em]{\textcolor{pilosa-red}{Cardinality 0}}
      }\\
      \vdots \\
      $\times \textcolor{pilosa-red}{N_c}$
    }\\
    \block[minimum width=16em]{
      Offset header\\
  ...
  \tikz\node at (6, 10) [thin,rounded corners,fill=white,text=pilosa-blue,align=center,minimum width=1mm] (containers) {
    ...
    \quad \block[minimum width=50mm]{
      RLE container\\
      2 \block[minimum width=13em]{\textcolor{pilosa-red}{$N_{runs}$}}\\
